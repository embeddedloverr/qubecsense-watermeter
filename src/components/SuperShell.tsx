...
import { IconDashboard, IconHome, IconUsers, IconLogout } from "./icons";

// Shell for the superadmin area. Deliberately distinct from AppShell — this is
// the cross-site view, not a site's own nav.

const nav = [
  { href: "/superadmin", label: "Overview", icon: IconDashboard },
  { href: "/superadmin/sites", label: "Sites", icon: IconHome },
  { href: "/superadmin/admins", label: "Admins", icon: IconUsers },
];
      ...
      <header className="sticky top-0 z-40 border-b border-border bg-card/85 backdrop-blur supports-[backdrop-filter]:bg-card/70">
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4">
          <div className="flex items-center gap-3">
            <Logo />
            <span className="hidden rounded-full bg-warning/15 px-2.5 py-0.5 text-[11px] font-semibold uppercase tracking-wide text-warning sm:inline">
              Superadmin
            </span>
          </div>
